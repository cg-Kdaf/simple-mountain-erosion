...
float getDisplacement(float2 uv) {
    return (cos(uv.x * 10.0) + sin(uv.y * 10.0)) * 0.1 + (cos(uv.x * 100.0) + sin(uv.y * 100.0)) * 0.02;
}
  ...
  uint2 gid [[thread_position_in_grid]])
{
  ...
  // Estimate derivatives using central differences in UV space
  float du = 1.0 / float(width);
  float dv = 1.0 / float(height);
  float hL = getDisplacement(uv + float2(-du, 0.0));
  float hR = getDisplacement(uv + float2( du, 0.0));
  float hD = getDisplacement(uv + float2(0.0, -dv));
  float hU = getDisplacement(uv + float2(0.0,  dv));

  float dhdx = (hR - hL) * 0.5;
  float dhdy = (hU - hD) * 0.5;
  float scale = 100.0; // match vertex displacement scale
